...
## Purpose
Provides a FastAPI HTTP service that exposes risk scoring functionality for consumer financial profiles. The current version returns deterministic mock scores while establishing the public contract (endpoints, schemas, error semantics) that future ML-backed versions MUST preserve.
